...
async function signup({email ,accountName, password , cart = []}) {
    const saltRounds = 10
    ...
    if (userExist) return Promise.reject('Username already exists')

    const hash = await bcrypt.hash(password, saltRounds)
    return userService.add({email, accountName, password: hash , cart})
}
